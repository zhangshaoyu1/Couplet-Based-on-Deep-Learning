...
class Model():
    # 参数初始化
    # save_step:step步保存一次模型
    # eval_step:eval步评估一次模型
    def __init__(self, train_input_file, train_target_file,
        ...
        if init_infer:  # 初始化infer
            self.infer_vocabs = reader.read_vocab(vocab_file)  # infer_vocabs词表文件
    ...
    def train(self, epochs, start=0):  # 训练
        ...
        with self.train_graph.as_default():
            if path.isfile(self.model_file + '.meta') and self.restore_model:  # 是否读取上一次的模型
    ...
    def infer(self, text):  # 调用模型根据上联计算下联
